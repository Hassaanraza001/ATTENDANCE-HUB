    ...
    for attempt in range(5):
        try:
            ...
            if finger.read_sysparam() == adafruit_fingerprint.OK:
                # Set security level to minimum for matching logic
...
def capture_single_finger_model(label, student_name):
    """Helper to capture 2 scans for a single finger and merge into one char buffer"""
...
def enroll(student_id, user_id, student_name):
    ...
    with sensor_lock:
        ...
        try:
            ...
            # Save Finger 1 Immediately
            f1_path = os.path.join(TEMPLATES_DIR, f"{student_id}_f1.dat")
            with open(f1_path, "wb") as f: f.write(bytearray(model1))
            ...
            # Save Finger 2 Immediately
            f2_path = os.path.join(TEMPLATES_DIR, f"{student_id}_f2.dat")
            with open(f2_path, "wb") as f: f.write(bytearray(model2))
    ...
    with sensor_lock:
        try:
            ...
            for doc in docs:
                ...
                # Check for separate finger files
            ...
            print(f"Synced {len(slot_map)} slots for {target_class}")
        except Exception: pass
                    ...
                    if finger.image_2_tz(1) == adafruit_fingerprint.OK:
                        if finger.finger_search() == adafruit_fingerprint.OK:
                            ...
                            if student_id and db and cached_user_id:
                                ...
                                current_attendance = doc_ref.get().to_dict().get("attendance", {})
                                
                                if current_attendance.get(today) == "present":
                                    db.collection(COLLECTION_STATUS).document(DEVICE_SERIAL).update({
                                        "scan_status": "already-present", 
